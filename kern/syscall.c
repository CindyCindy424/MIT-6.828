...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	tf->tf_eflags = FL_IF;
	tf->tf_eflags &= ~FL_IOPL_MASK;  //不同进程不可以有IO权限
	tf->tf_cs = GD_UT | 3;
	e->env_tf = *tf;
	return 0;
}
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	struct Env *e; 	
	int ret = envid2env(envid, &e, 1);  //由envid 得到Env *e
	if (ret) return ret;	//error

	if ((va >= (void*)UTOP) || (ROUNDDOWN(va, PGSIZE) != va)) return -E_INVAL;  //va >= UTOP, or va is not page-aligned.	
	int flag = PTE_U | PTE_P;
	if ((perm & flag) != flag) return -E_INVAL;  //perm is inappropriate

	struct PageInfo *pg = page_alloc(1);			//分配物理页
	if (!pg) return -E_NO_MEM;
	ret = page_insert(e->env_pgdir, pg, va, perm);	//建立映射关系
	if (ret) {
		page_free(pg);  //页面置为空白=0
		return ret;
	}
	return 0;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	struct Env *se, *de;   //source & destination
...
sys_page_unmap(envid_t envid, void *va)
{
	...
	if ((va >= (void*)UTOP) || (ROUNDDOWN(va, PGSIZE) != va)) return -E_INVAL; //va >= UTOP, or va is not page-aligned
	page_remove(env->env_pgdir, va);   //umap
	return 0;
}
